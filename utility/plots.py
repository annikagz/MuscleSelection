...
def plot_the_predictions(model, saved_model_path, saved_model_name, x_test, y_test, lstm_hidden_size=16):
    ...
    with torch.no_grad():
        ...
        for rep in range(x_test.shape[-1]):
            h = torch.zeros(1, x_test.shape[1], lstm_hidden_size).to(device)
            c = torch.zeros(1, x_test.shape[1], lstm_hidden_size).to(device)
            hidden = tuple([h, c])
